...
$(document).ready(function() {
    ...
    // Manejar envío del formulario de filtros de historial de caja
    $(document).on('submit', '#filtroFormHistorialCaja', function(e) {
        e.preventDefault();
        var formData = $(this).serialize();
        var actionUrl = $(this).attr('action');
        cargarContenido(actionUrl + '?' + formData);
    ...
    // Manejar botón limpiar filtro de historial de caja
    $(document).on('click', '#btnLimpiarHistorialCaja', function(e) {
        e.preventDefault();
        // Limpiar todos los campos del formulario
        $('#filtroFormHistorialCaja')[0].reset();
        // Recargar la vista de caja sin filtros
